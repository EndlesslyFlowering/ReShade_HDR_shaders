...
namespace CSP
{
  ...
  namespace TRC
  {
    ...
    // (EOTF) takes HLG values as input
    float3 FromHlg(float3 x)
    {
      return float3(FromHlg(x.r),
                    FromHlg(x.g),
                    FromHlg(x.b));
    }
  ...
  namespace YCbCr
  {
    ...
    namespace ToRGB
    {
      ...
      {
        return float3(
          Colour.x + K_Helpers::BT709::KR    * Colour.z,
          Colour.x - K_Helpers::BT709::KG[0] * Colour.y - K_Helpers::BT709::KG[1] * Colour.z,
          Colour.x + K_Helpers::BT709::KB    * Colour.y);
      }

      float3 BT2020(const float3 Colour)
      {
        return float3(
          Colour.x + K_Helpers::BT2020::KR    * Colour.z,
          Colour.x - K_Helpers::BT2020::KG[0] * Colour.y - K_Helpers::BT2020::KG[1] * Colour.z,
          Colour.x + K_Helpers::BT2020::KB    * Colour.y);
      }

      float3 AP0_D65(const float3 Colour)
      {
        return float3(
          Colour.x + K_Helpers::AP0_D65::KR    * Colour.z,
          Colour.x - K_Helpers::AP0_D65::KG[0] * Colour.y - K_Helpers::AP0_D65::KG[1] * Colour.z,
          Colour.x + K_Helpers::AP0_D65::KB    * Colour.y);
      }
    ...
    namespace FromRGB
    {
      ...
      float3 BT709(const float3 Colour)
      {
        const float Y = dot(Colour, K_Helpers::BT709::K);
        return float3(Y,
                      (Colour.b - Y) / K_Helpers::BT709::KB,
                      (Colour.r - Y) / K_Helpers::BT709::KR);
      }

      float3 BT2020(const float3 Colour)
      {
        const float Y = dot(Colour, K_Helpers::BT2020::K);
        return float3(Y,
                      (Colour.b - Y) / K_Helpers::BT2020::KB,
                      (Colour.r - Y) / K_Helpers::BT2020::KR);
      }

      float3 AP0_D65(const float3 Colour)
      {
        const float Y = dot(Colour, K_Helpers::AP0_D65::K);
        return float3(Y,
                      (Colour.b - Y) / K_Helpers::AP0_D65::KB,
                      (Colour.r - Y) / K_Helpers::AP0_D65::KR);
      }
  ...
  namespace Mat
  {
    ...
    namespace BT709To
    {
      ...
      {
        return mul(BT709_To_XYZ, Colour);
      }

      float3 DCI_P3(const float3 Colour)
      {
        return mul(BT709_To_DCI_P3, Colour);
      }

      float3 BT2020(const float3 Colour)
      {
        return mul(BT709_To_BT2020, Colour);
      }

      float3 AP0_D65(const float3 Colour)
      {
        return mul(BT709_To_AP0_D65, Colour);
      }
    } //BT709To
    ...
    namespace BT2020To
    {
      float3 XYZ(const float3 Colour)
      {
        return mul(BT2020_To_XYZ, Colour);
      }

      float3 BT709(const float3 Colour)
      {
        return mul(BT2020_To_BT709, Colour);
      }

      float3 AP0_D65(const float3 Colour)
      {
        return mul(BT2020_To_AP0_D65, Colour);
      }
    } //BT2020To
    ...
    namespace AP0_D65To
    {
      float3 XYZ(const float3 Colour)
      {
        return mul(AP0_D65_To_XYZ, Colour);
      }

      float3 BT709(const float3 Colour)
      {
        return mul(AP0_D65_To_BT709, Colour);
      }

      float3 BT2020(const float3 Colour)
      {
        return mul(AP0_D65_To_BT2020, Colour);
      }
    } //AP0_D65To
    ...
    namespace AP1_D65To
    {
      float3 XYZ(const float3 Colour)
      {
        return mul(AP1_D65_To_XYZ, Colour);
      }
    } //AP1_D65To
    ...
    namespace XYZTo
    {
      float3 BT709(const float3 Colour)
      {
        return mul(XYZ_To_BT709, Colour);
      }

      float3 DCI_P3(const float3 Colour)
      {
        return mul(XYZ_To_DCI_P3, Colour);
      }

      float3 BT2020(const float3 Colour)
      {
        return mul(XYZ_To_BT2020, Colour);
      }

      float3 AP1(const float3 Colour)
      {
        return mul(XYZ_To_AP1, Colour);
      }

      float3 AP0(const float3 Colour)
      {
        return mul(XYZ_To_AP0, Colour);
      }
    } //XYZTo
  ...
  namespace ICtCp
  {
    ...
    namespace Mat
    {
      ...
      //L'M'S'->ICtCp
      static const float3x3 PQ_LMS_To_ICtCp = float3x3(
        0.5,             0.5,             0.0,
        1.61376953125,  -3.323486328125,  1.709716796875,
        4.378173828125, -4.24560546875,  -0.132568359375);

      //ICtCp->L'M'S'
      static const float3x3 ICtCp_To_PQ_LMS = float3x3(
      ...
      //RGB BT.2020->LMS
      static const float3x3 BT2020_To_LMS = float3x3(
      ...
      //RGB AP0_D65->LMS
      static const float3x3 AP0_D65_To_LMS = float3x3(
      ...
      //LMS->RGB BT.2020
      static const float3x3 LMS_To_BT2020 = float3x3(
      ...
      //LMS->RGB AP0_D65
      static const float3x3 LMS_To_AP0_D65 = float3x3(
      ...
      namespace ICtCpTo
      {
        ...
        {
          return mul(ICtCp_To_PQ_LMS, Colour);
        }
      } //ICtCpTo
      ...
      namespace PqLmsTo
      {
        ...
        {
          return mul(PQ_LMS_To_ICtCp, Colour);
        }
      } //PqLmsTo
      ...
      namespace BT2020To
      {
        ...
        {
          return mul(BT2020_To_LMS, Colour);
        }
      } //BT2020To
      ...
      namespace AP0_D65To
      {
        ...
        float3 LMS(const float3 Colour)
        {
          return mul(AP0_D65_To_LMS, Colour);
        }
      } //AP0_D65To
      ...
      namespace LmsTo
      {
        ...
        float3 BT2020(const float3 Colour)
        {
          return mul(LMS_To_BT2020, Colour);
        }

        //LMS->RGB AP0_D65
        float3 AP0_D65(const float3 Colour)
        {
          return mul(LMS_To_AP0_D65, Colour);
        }
      } //LmsTo
    ...
    namespace NormalisedToIntensity
    {
      ...
      float BT2020(const float Normalised)
      {
        ...
        float2 LmNormalised = float2(dot(normalised3, Mat::BT2020_To_LMS[0]),
                                     dot(normalised3, Mat::BT2020_To_LMS[1]));
      ...
      float AP0_D65(const float Normalised)
      {
        float3 normalised3 = float3(Normalised, Normalised, Normalised);

        float2 LM_normalised = float2(dot(normalised3, Mat::AP0_D65_To_LMS[0]),
                                      dot(normalised3, Mat::AP0_D65_To_LMS[1]));

        float2 LmPq = TRC::ToPq(LM_normalised);
    ...
    namespace NitsToIntensity
    {
      ...
      float AP0_D65(const float nits)
      {
        return NormalisedToIntensity::AP0_D65(nits / 10000.f);
      }
